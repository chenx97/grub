  ...
  for (i = 0, s = smd->sections;
	...
	for (j = 0, r = (Elf_Rela *) ((char *) e + rtab_offset);
	     ...
	     j++, r = (Elf_Rela *) ((char *) r + r_size))
	  {
	    ...
	    /* mips64 has a cursed definition of r_info:
	     * 0: Elf32_Word     r_sym;
	     * 32: unsigned char r_ssym;
	     * 40: unsigned char r_type3;
	     * 48: unsigned char r_type2;
	     * 56: unsigned char r_type1;
	     * Although we will only use r_sym and r_type1, we treat swapped
	     * upper fields as a single word so we can report an error when
	     * r_type2 is nonzero.
	     */
	    if (image_target->elf_target == EM_MIPS && image_target->voidp_sizeof == 8)
	      info = ELF64_R_INFO (r->r_info, (grub_uint32_t) grub_be_to_cpu64(r->r_info));
	    else
	      info = grub_target_to_host (r->r_info);
		...
		default:
		 ...
		 switch (ELF_R_TYPE (info))
		   ...
		   case R_MIPS_26:
		     {
		       grub_uint32_t *t32 = (grub_uint32_t *) target;
		       *t32 = grub_host_to_target32 ((grub_target_to_host32 (*t32) & 0xfc000000U) |
						     ((sym_addr >> 2) & 0x3ffffffULL));
		     }
		     break;
		   case R_MIPS_LO16:
		     {
		       grub_int16_t *t16 = (grub_int16_t *) target;
		       *t16 = grub_host_to_target16 (sym_addr);
		     }
		     break;
		   case R_MIPS_HI16:
		     {
		       grub_int16_t *t16 = (grub_int16_t *) target;
		       *t16 = grub_host_to_target16 ((sym_addr + 0x8000ULL) >> 16);
		     }
		     break;
		   case R_MIPS_HIGHER:
		     {
		       grub_int16_t *t16 = (grub_int16_t *) target;
		       *t16 = grub_host_to_target16 ((sym_addr + 0x80008000ULL) >> 32);
		     }
		     break;
		   case R_MIPS_HIGHEST:
		     {
		       grub_uint16_t *t16 = (grub_uint16_t *) target;
		       *t16 = grub_host_to_target16 ((sym_addr + 0x800080008000ULL) >> 48);
		     }
		     break;
		   default:
		     grub_util_error (_("relocation 0x%x is not implemented yet"),
	...
	case R_MIPS_LO16:
	  {
	    ...
	    ctx->current_address
	      = add_fixup_entry (&ctx->lst,
				 ...
				 image_target);
	    /* Hi */
	    grub_util_info ("appending an R_MIPS_HI16 entry at 0x%"
			    GRUB_HOST_PRIxLONG_LONG,
			    ((target + 0x8000ULL) >> 16) & 0xffff);
	    ctx->current_address
	    ...
	    grub_util_info ("appending an R_MIPS_HIGHER entry at 0x%"
			    GRUB_HOST_PRIxLONG_LONG,
			    ((target + 0x80008000ULL) >> 32) & 0xffff);
	    /* Higher */
	    ctx->current_address
	      = add_fixup_entry (&ctx->lst, 0,
	    ...
	    grub_util_info ("appending an R_MIPS_HIGHEST entry at 0x%"
			    GRUB_HOST_PRIxLONG_LONG,
			    ((target + 0x800080008000ULL) >> 48) & 0xffff);
	    /* Highest */
	    ctx->current_address
	      = add_fixup_entry (&ctx->lst, 0,
	...
	case R_MIPS_HI16:
	  {
	    ...
	    ctx->current_address
	      = add_fixup_entry (&ctx->lst,
				 ...
				 image_target);
	    /* Lo */
	    grub_util_info ("appending an R_MIPS_LO16 entry at 0x%"
			    GRUB_HOST_PRIxLONG_LONG,
			    (unsigned long long) target & 0xffff);
	    ctx->current_address
	      = add_fixup_entry (&ctx->lst, 0,
				 (grub_int16_t) target,
				 0, ctx->current_address,
	...
	case R_MIPS_HIGHER:
	  {
	    ...
	    grub_util_info ("appending an R_MIPS_LO16 entry at 0x%"
			    GRUB_HOST_PRIxLONG_LONG,
			    (unsigned long long) (grub_int16_t) target);
	    ctx->current_address
	      = add_fixup_entry (&ctx->lst, 0,
				 (grub_int16_t) target,
				 0, ctx->current_address,
